...
### Stage T — Translation (отдельный pipeline, после утверждения русской главы)

Русская глава считается исходником для перевода только после прохождения Stage C/D без блокеров. Русский файл при переводе не изменяется ни на байт.

1. **T1 — Translator** (`agents/translator.md`): строгий перевод главы в `chapters/en/NN-slug.md` (или глоссария в `glossary.en.md`). Markdown-структура сохраняется байт в байт; фронматтер отличается только `title` (переведён) и `language: en`; якоря `[^g-id]`/`{#g-id}` не меняются. Глоссарий и русские исходники не трогаются.
2. **T2 — Translation Auditor** (`agents/translation-auditor.md`): независимая сверка перевода с исходником — полнота (каждая секция/таблица/сноска), структурный паритет, стабильность якорей, отсутствие добавлений/сокращений, терминологическая консистентность, отсутствие остаточного русского. Вердикт `PASS` или `REVISION REQUIRED` со списком findings.
3. **T3 — правки**: при `REVISION REQUIRED` Translator исправляет все findings одним проходом, затем повторный T2. Максимум два revision cycles; если FAIL остаётся — явно сообщить пользователю.

Перевод главы считается готовым только при вердикте `PASS`.
...
- `Глава N` — полный pipeline: Planner → Author → Auditor → Editor if needed → Stage T (Translator → Translation Auditor → правки) → final chapter.
...
- `Переведи главу N` — Stage T для готовой русской главы: Translator → Translation Auditor → правки при `REVISION REQUIRED` → `chapters/en/NN-slug.md`.
- `Переведи глоссарий` — Stage T для глоссария: `glossary.md` → `glossary.en.md`, якоря `{#g-id}` сохраняются.

Translation pipeline запускается только для главы, прошедшей Stage C/D без блокеров. Русский исходник не перезаписывать.
